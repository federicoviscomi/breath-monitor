...
public class DiscreteTimePlotter implements PlotterInterface {
	...
	public static void main(final String[] args) {
		final DiscreteTimePlotter p = new DiscreteTimePlotter();
	...
	private boolean timeBoundExceeded = false;
	 ...
	 * per il vecchio e uno per il nuovo e che si alternano.
	 */
	...
	public void plotBeatFound(final Color color) {
		if (time > timeBound) {
			if (!timeBoundExceeded) {
				System.err
						.println("\nWARNING: DiscreteTimePlotter.timeBound exceeded");
			}
			timeBoundExceeded = true;
		} else {
			// aggiungere un grafico diverso ma con la stessa scala delle
			// ascisse!
			final double[] x = { time };
			final double[] y = { 0.3 };
