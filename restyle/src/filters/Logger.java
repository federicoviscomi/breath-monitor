...
public class Logger {
	 ...
	 *            the message to log
	 */
	public static synchronized void error(final String msg) {
		println("ERROR:  " + msg);
	}
	 ...
	 */
	public static synchronized void exception(final String s, final Exception e) {
		error(s);
		e.printStackTrace();
		System.out.flush();
	}
	 ...
	 *            the message to log.
	 */
	public static synchronized void forcePrintln(final String msg) {
		if (bw != null) {
			writeFile(msg);
		}
	...
	public static String getDate() {
		...
		final String m = month[now.get(Calendar.MONTH)];
	...
	public static void init() {
		// Open log file
		logFileName = System.getProperty("com.sun.voip.server.BRIDGE_LOG",
				"bridge.log");
	...
	public static void init(final String logFileName,
			final boolean suppressSystemOut) {
		Logger.logFileName = logFileName;

		final String s = System.getProperty(LOG_LEVEL, "3");
	 ...
	 *            the message to log.
	 */
	public static synchronized void println(final String msg) {
		if (bw != null) {
			writeFile(msg);
	...
	/**
	 * Logs a message to the log file only. This write is buffered so as to not
	 * disrupt timing.
	 * 
	 * @param msg
	 *            the message to log.
	 */
	public static synchronized void writeFile(final String msg) {
		if (bw != null) {
			try {
				synchronized (bw) {
