...
public class WavFile {
	...
	public static void main(final String[] args) {
		...
		try {
			for (final String filename : args) {
				...
				final WavFile writeWavFile = newWavFile(new File("out.wav"),
			...
			final WavFile writeWavFile = newWavFile(new File("out2.wav"), 1,
					10, 23, 44100);
	...
	public static WavFile newWavFile(final File file, final int numChannels,
			final long numFrames, final int validBits, final long sampleRate)
	...
	public static WavFile openWavFile(final File file) throws IOException,
			WavFileException {
		...
		// System.err.println(wavFile.toString());

		return wavFile;
	}

	private static void putLE(long val, final byte[] buffer, int pos,
			final int numBytes) {
		for (int b = 0; b < numBytes; b++) {
	...
	public int readFrames(final double[] sampleBuffer, int offset,
			final int numFramesToRead) throws IOException, WavFileException {
		// System.err.print("WavFile.readFrames(" + offset + ", "
		// + numFramesToRead + ")[frameCounter=" + frameCounter
		// + ", nemFrames=" + numFrames + "]:");
		...
		for (int f = 0; f < numFramesToRead; f++) {
			...
			for (int c = 0; c < numChannels; c++) {
				sampleBuffer[offset] = floatOffset
		...
		// System.err.println(numFramesToRead + "");
		return numFramesToRead;
	}

	public int readFrames(final double[][] sampleBuffer,
			final int numFramesToRead) throws IOException, WavFileException {
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}
	...
	public int readFrames(final int[] sampleBuffer, int offset,
			final int numFramesToRead) throws IOException, WavFileException {
		if (ioState != IOState.READING) {
			throw new IOException("Cannot read from WavFile instance");
		}
	...
	public int readFrames(final long[] sampleBuffer, int offset,
			final int numFramesToRead) throws IOException, WavFileException {
		if (ioState != IOState.READING) {
			throw new IOException("Cannot read from WavFile instance");
		}
	...
	public String toString() {
		return "file:" + file + ",ioState:" + ioState + ",bytesPerSample:"
				+ bytesPerSample + ",numFrames:" + numFrames + ",floatScale:"
				+ floatScale + ",floatOffset:" + floatOffset
				+ ",wordAlignAdjust:" + wordAlignAdjust + ",numChannels:"
				+ numChannels + ",sampleRate:" + sampleRate + ",blockAlign:"
				+ blockAlign + ",validBits:" + validBits + ",bufferPointer:"
				+ bufferPointer + ",bytesRead:" + bytesRead + ",frameCounter:"
				+ frameCounter;
	}

	public int writeFrames(final double[] sampleBuffer,
			final int numFramesToWrite) throws IOException {
	...
	public int writeFrames(final int[] sampleBuffer, int offset,
			final int numFramesToWrite) throws IOException {
		if (ioState != IOState.WRITING) {
			throw new IOException("Cannot write to WavFile instance");
		}
	...
	public int writeFrames(final int[][] sampleBuffer,
			final int numFramesToWrite) throws IOException {
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}
	...
	public int writeFrames(final long[] sampleBuffer, int offset,
			final int numFramesToWrite) throws IOException {
		if (ioState != IOState.WRITING) {
			throw new IOException("Cannot write to WavFile instance");
		}
	...
	public int writeFrames(final long[][] sampleBuffer,
			final int numFramesToWrite) throws IOException {
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}
